...
const CashChargeForm = () => {
  ...
  useEffect(() => {
    ...
    if (encrypted) {
      try {
        ...
        if (value) setCurrentCash(Number(value));
      } catch (err) {
        console.error('Failed to decrypt cached cash', err);
      }
    }
  }, []);
  ...
  useEffect(() => {
    const encrypted = CryptoJS.AES.encrypt(String(currentCash), SECRET_KEY).toString();
