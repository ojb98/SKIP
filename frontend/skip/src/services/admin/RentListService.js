...
export const fetchWithdrawRents = async () => {
  const response = await axios.get('/api/rent/withdraw');
  return response.data;
};
// 전체 사용자 조회 - 승인
export const fetchApprovalRents = async () => {
  const response = await axios.get('/api/rent/approval');
  return response.data;
};
// 사용자ID 기준 조회
export const findRentByUserId = async (userid) => {
  const response = await axios.get(`/api/rent/find/userid/${userid}`);
  return Array.isArray(response.data) ? response.data : [response.data];
};

// 이름 기준 조회
export const findRentByName = async (username) => {
  const response = await axios.get(`/api/rent/find/username/${username}`);
  return Array.isArray(response.data) ? response.data : [response.data];
};

// 렌탈샵이름 기준 조회
export const findRentByRentName = async (rentname) => {
  const response = await axios.get(`/api/rent/find/rentname/${rentname}`);
  return Array.isArray(response.data) ? response.data : [response.data];
};
