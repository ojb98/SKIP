...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query("SELECT p.createdAt AS date, SUM(p.totalPrice) AS total FROM Payment p WHERE p.createdAt BETWEEN :atStart AND :atEnd GROUP BY p.createdAt ORDER BY p.createdAt")
    List<Map<String, Object>> getDailySales(@Param("atStart") String atStart, @Param("atEnd") String atEnd);

    @Query("SELECT p.method AS method, SUM(p.totalPrice) AS total FROM Payment p WHERE p.createdAt BETWEEN :atStart AND :atEnd GROUP BY p.method ORDER BY p.method")
    List<Map<String, Object>> getCategorySales(@Param("atStart") String atStart, @Param("atEnd") String atEnd);
}